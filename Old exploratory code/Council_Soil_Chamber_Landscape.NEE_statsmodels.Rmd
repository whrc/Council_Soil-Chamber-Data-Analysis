...
landscape_wideNEE <- landscapeinun_NEE_summary_rounded3 %>%
  pivot_wider(
  ...
  landscape_wideNEE <- landscape_wideNEE %>%
  mutate(
    flux_value_mean_pct_change = ((abs(flux_value_mean_Y) - abs(flux_value_mean_N)) / abs(flux_value_mean_N)) * 100, #absolute value of flux to correctly calc the % change 
    flux_value_umol_mean_pct_change = ((abs(flux_value_umol_mean_Y) - abs(flux_value_umol_mean_N)) / abs(flux_value_umol_mean_N)) * 100, #absolute value of flux 
...
percent_change_summary <- landscape_wideNEE %>%
...
vif(NEE.final2) # all <2, so should be ok! 
```
...
# create first qq plot using model residuals
# color it red
qqnorm(residuals(NEE.final2), xlab = "", ylab = "", main = "", 
       col = "red")
qqline(residuals(NEE.final2))

# now create 24 qq plots using Normal data with sigma(dataset)
for(i in 1:24){
  # rnorm() samples from a Normal distribution  
  d <- rnorm(length(residuals(NEE.final2)), 
             mean = 0, sd = sigma(NEE.final2))
  qqnorm(d, xlab = "", ylab = "", main = "")
  qqline(d)
}
...
#comparing data QQplot to a normal QQplot and hist 
qqnorm(residuals(NEE.final2)) #normality of residuals 
hist(residuals(NEE.final2)) 
car::qqPlot(x = residuals(NEE.final2)) #shows where residuals breach normal distr
qqPlot(residuals(NEE.final2))
```
...
# Histogram of residuals-NEEfinal2
hist(std_resid2, main = "Histogram of Standardized Residuals", 
     xlab = "Standardized Residuals", freq = FALSE)
# Superimpose best fitting normal curve
curve(dnorm(x, mean = mean(std_resid2, na.rm=T), sd = sd(std_resid2, na.rm=T)),
      lty = 2, lwd = 2, add = TRUE, col = "blue")
# Superimpose empirical density (no normality assumption, for comparison)
lines(density(std_resid2, na.rm=T), lwd = 2, col = "red")


#~~~~using raw residuals from NEE.final2, not standardized residuals, just to check ~~~~~~~~~~~

RESID <- NEE.final2$residuals

length(RESID) #158
summary(NEE.final2) #6 predictors 
#158/6 = 26, which is over 10, which some lit says doesn't need to concern itself with normality....?
...
# Formal test
shapiro_test3 <- shapiro.test(RESID)
print(shapiro_test3) #for gls final model: p<0.001; normality NOT supported 
...
Anova(robust_model_all, type = "II") #landpos p=0.056, marginal*, airtemp p = 0.0007 *SIG, thawdepth p = 0.004 *SIG, VWC p = 0.01 *SIG
...
cor_matrix2 <- cor(chamber_NEE_corr_matrix, method = "spearman", use = "complete.obs")
...
# Fig: only labels corr values above 0.3 
...
#Further exploration, transformation, glm comparison to robust reg with Claude 
```{r}
# First, let's examine the distribution of your NEE flux data
library(ggplot2)
library(MASS)  # For robust regression
library(nlme)  # For GLS
library(car)   # For transformations

# Histogram of NEE fluxes
ggplot(df_NEE, aes(x = flux_value)) +
  geom_histogram(bins = 30, fill = "steelblue", color = "black") +
  labs(title = "Distribution of NEE Flux Values",
       x = "NEE Flux",
       y = "Frequency") +
  theme_minimal()
...
qqnorm(df_NEE$flux_value)
qqline(df_NEE$flux_value)

# Try different transformations
# Create a data frame to store transformed values
transforms <- data.frame(
  original = df_NEE$flux_value,
  log_abs = sign(df_NEE$flux_value) * log(abs(df_NEE$flux_value) + 1),  # Sign-preserving log
  cube_root = sign(df_NEE$flux_value) * abs(df_NEE$flux_value)^(1/3),   # Cube root (preserves sign)
  yeo_johnson = NA  # Will fill this in below
)

# Yeo-Johnson transformation (handles negative values)
# Find optimal lambda
yj_transform <- car::powerTransform(df_NEE$flux_value, family = "yjPower")
lambda <- yj_transform$lambda
transforms$yeo_johnson <- car::yjPower(df_NEE$flux_value, lambda)

# Plot histograms of all transformations
library(tidyr)
library(dplyr)

transforms_long <- transforms %>%
  pivot_longer(cols = everything(), 
               names_to = "transformation", 
               values_to = "value")

ggplot(transforms_long, aes(x = value)) +
  geom_histogram(bins = 30, fill = "steelblue", color = "black") +
  facet_wrap(~transformation, scales = "free") +
  labs(title = "Comparison of Transformations for NEE Flux",
       x = "Transformed Value",
       y = "Frequency") +
  theme_minimal()

# Test normality of each transformation
transformations <- c("original", "log_abs", "cube_root", "yeo_johnson")
normality_tests <- lapply(transformations, function(trans) {
  shapiro.test(transforms[[trans]])
})
names(normality_tests) <- transformations

# Print results
for(i in seq_along(normality_tests)) {
  cat("Shapiro-Wilk test for", names(normality_tests)[i], "transformation:\n")
  print(normality_tests[[i]])
  cat("\n")
}

#Yeo-johnson seems to drastically improve normality, making it look almost normal, but ALL transformations fail the shapiro-wilkes test with highly significant p values

#going to try adding it, just to see if it improves residuals 


# Based on the best transformation, fit models:

# 1. Robust regression (your current approach)
robust_model <- rlm(flux_value ~ landscape_position + VWC + soil_temp_10_cm + 
                    air_temp + thawdepth + inundated,
                    data = df_NEE)

# 2. GLM with Gaussian family
glm_model <- glm(flux_value ~ landscape_position + VWC + soil_temp_10_cm + 
                air_temp + thawdepth + inundated,
                family = gaussian(link = "identity"),
                data = df_NEE)

# 3. GLM with best transformation (assuming yeo_johnson worked best)
# Add the transformed response to the dataset
df_NEE$flux_transformed <- transforms$yeo_johnson

glm_transformed <- glm(flux_transformed ~ landscape_position + VWC + soil_temp_10_cm + 
                      air_temp + thawdepth + inundated,
                      family = gaussian(link = "identity"),
                      data = df_NEE)

# Compare model performance
# For robust model, we'll use a custom function to calculate pseudo-R²
robust_r2 <- function(model) {
  1 - (sum(model$weights * model$resid^2) / sum(model$weights * (model$y - mean(model$y))^2))
}
...
rlm_AIC(robust_model)


# Create a comparison table
model_comparison <- data.frame(
  Model = c("Robust Regression", "GLM (untransformed)", "GLM (transformed)"),
  AIC = c(rlm_AIC(robust_model), AIC(glm_model), AIC(glm_transformed)),
  BIC = c(NA, BIC(glm_model), BIC(glm_transformed)),
  R_squared = c(robust_r2(robust_model), 
                with(summary(glm_model), 1 - deviance/null.deviance),
                with(summary(glm_transformed), 1 - deviance/null.deviance))
)

# Print comparison
print(model_comparison) #robust regression has lowest AIC 

# Examine residuals
par(mfrow = c(3,2))
# Robust model
plot(fitted(robust_model), residuals(robust_model), 
     main = "Robust Model Residuals", xlab = "Fitted", ylab = "Residuals")
abline(h = 0, lty = 2)
qqnorm(residuals(robust_model)); qqline(residuals(robust_model))

# GLM without transformation
plot(fitted(glm_transformed), residuals(glm_model), 
     main = "GLM Residuals", xlab = "Fitted", ylab = "Residuals")
abline(h = 0, lty = 2)
qqnorm(residuals(glm_model)); qqline(residuals(glm_model))

# GLM with transformation
plot(fitted(glm_transformed), residuals(glm_transformed), 
     main = "Transformed GLM Residuals", xlab = "Fitted", ylab = "Residuals")
abline(h = 0, lty = 2)
qqnorm(residuals(glm_transformed)); qqline(residuals(glm_transformed))

# Check for significant predictors in the best model
summary(robust_model)
summary(glm_transformed)

Anova(robust_model, type = "II") 
Anova(glm_transformed, type = "II") 
...
#suggests VWC and thawdepth vary significantly among landpos; and all variables except for air temp vary significantly by inundation 

interaction_model1 <- rlm(flux_value ~ landscape_position * VWC + landscape_position * thawdepth + soil_temp_10_cm + air_temp + inundated, data = df_NEE)
Anova(interaction_model1, type = "II") 
vif(interaction_model1) #high VIF for landpos & interaction terms, but the adj VIR all 2.5 or below 

# Response: flux_value
#                               Df       F    Pr(>F)    
# landscape_position             2  3.3224 0.0388123 *  
# VWC                            1  4.5236 0.0351114 *  
# thawdepth                      1  8.4059 0.0043180 ** 
# soil_temp_10_cm                1  2.5702 0.1110540    
# air_temp                       1 12.5053 0.0005442 ***
# inundated                      1  0.2241 0.6366249    
# landscape_position:VWC         2  1.0469 0.3536357    
# landscape_position:thawdepth   2  0.8948 0.4109337    
# Residuals                    146  
```
#PCA for thawdepth, VWC, and landpos 
