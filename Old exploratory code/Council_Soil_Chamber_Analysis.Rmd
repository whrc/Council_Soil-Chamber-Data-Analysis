...
landscape_C_fluxes <- ggplot(df_long_cc, aes(x = landscape_position, y = (flux_value *1/12 * 1000000), fill = flux_type)) +
  ...
  labs(title = "Council Soil Chamber GPP, NEE, and RECO by Landscape Position",
       ...
       #y = expression(Flux~Value~(gCO[2]-C~m^-2~s^-1)),
        y = expression("Flux ("*mu* "mol" ~ CO[2]-C/m^2/s*")"),
...
```{r}
 ggsave(
  filename = "C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/Soil Chambers/landscape_C_fluxes2.png",  # File name and extension
...
#AGU figs
```{r}
#looking at FCH4 (in g/m2/s) by landscape position and plot type, facet wrapped --> revert to umolCO2 and nmolCH4* --> actually made FCH4 to umol 
ggplot(df_long,
...
landscape_CH4_fluxes <- ggplot(df_long, aes(x = landscape_position, y = (FCH4 *1/12 * 1000000), fill = inundated)) +
  ...
  labs(title = "Methane Fluxes vs Landscape Position and Inundation",
       ...
       #y = expression(FCH[4]~"(" * mu * "mol" ~ CH[4]-C~m^-2~s^-1),
       #y = expression(FCH[4]~(CH[4]-C~μmol~m^{-2}~s^{-1})),
       y = expression("Flux ("*mu* "mol" ~ CH[4]-C/m^2/s*")"),
       fill = "Inundated")+
   geom_hline(yintercept = 0)+
 scale_y_continuous(limits = c(-0.15,0.46), #cut out a few bit outliers here to show trend better, -0.25 and 0.5 looks good but trying others 
  breaks = seq(-0.15, 0.46, 0.15))+
...
```{r}
ggsave(
  filename = "C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/Soil Chambers/landscape_CH4_fluxes2.png",  # File name and extension
