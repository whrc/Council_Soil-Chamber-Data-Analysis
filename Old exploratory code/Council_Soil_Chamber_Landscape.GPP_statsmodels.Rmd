...
```{r}
library(dplyr)
#GPP is in g/m2/s, convert back to umol (?) --> flux_value_umol
df_GPP$flux_value_umol <- df_GPP$flux_value *1/12 * 1000000 #same as flux_CO2

#generates Std.Error and Std.Dev for avgs of each site type to use for ggplot & 95% confidence interval calcs

landscape_GPP_summary <- df_GPP %>%
...
landscape_GPP_summary_rounded <- landscape_GPP_summary  %>%
...
landscape_GPP_summary_rounded2 <- landscape_GPP_summary  %>%
...
landscape_GPP_summary_rounded3 <- landscape_GPP_summary %>%
  mutate(
    across(
...
write.csv(landscape_GPP_summary_rounded3, "C:/Users/kkent/Documents/Council Data/Soil Chambers_Council/landscape_GPPflux_summary_rounded.csv", row.names = FALSE)
...
landscapeinun_GPP_summary <- df_GPP %>%
  select(landscape_position, inundated, flux_value, flux_value_umol, thawdepth, soil_temp_10_cm, air_temp, VWC) %>%
...
#format table, round to 2 decimal places --> this rounds flux value into 0.00 so we need to exclude it 
landscapeinun_GPP_summary_rounded <- landscapeinun_GPP_summary  %>%
  mutate(across(where(is.numeric), ~round(., 2)))
...
#OR use this, to round to 2 sig digits
landscapeinun_GPP_summary_rounded2 <- landscapeinun_GPP_summary  %>%
  mutate(across(where(is.numeric), ~signif(., 3)))


#combine to round enviro vars to 2 decimals and flux values to 2 sig digits so they're all formatted similarly in output
landscapeinun_GPP_summary_rounded3 <- landscapeinun_GPP_summary %>%
  mutate(
    across(
      where(is.numeric) & !starts_with("flux_value"),
      ~round(., 2)
...
write.csv(landscapeinun_GPP_summary_rounded3, "C:/Users/kkent/Documents/Council Data/Soil Chambers_Council/landscapeinun_GPPflux_summary_rounded.csv", row.names = FALSE)
...
landscapeinun_GPP_summary_combined <- landscapeinun_GPP_summary_rounded3%>%
    mutate(
...
write.csv(landscapeinun_GPP_summary_combined, "C:/Users/kkent/Documents/Council Data/Soil Chambers_Council/landscapeinun_GPPflux_summary_combined2.csv", row.names = FALSE)
...
landscape_wideGPP<- landscapeinun_GPP_summary_rounded3 %>%
  pivot_wider(
  ...
  landscape_wideGPP <- landscape_wideGPP %>%
  mutate(
...
cor_matrix2 <- cor(chamber_GPP_corr_matrix, method = "spearman", use = "complete.obs")
