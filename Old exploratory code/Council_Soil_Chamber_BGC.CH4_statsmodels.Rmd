---
title: "Council_Soil_Chamber_BGC.CH4"  #FCH4 in gC/m2/s, flux_CH4 is in nano mol/m2/s
...
#can check how many are missing in each var 
# sum(is.na(df_BGC$flux_CH4)) #
# sum(is.na(df_BGC$thawdepth)) #
# sum(is.na(df_BGC$soil_temp_10_cm)) #
# sum(is.na(df_BGC$VWC)) #
# sum(is.na(df_BGC$air_temp)) # 

#use complete cases 
df_BGC <- df_BGC %>%
   filter(complete.cases(flux_value, thawdepth, soil_temp_10_cm,landscape_position, inundated, VWC, air_temp)) #went from 260 to 204 obs 
...
qqnorm(df_BGC$flux_value)
qqline(df_BGC$flux_value, col = "red")


#histograms with density curves by landscape position
ggplot(df_BGC, aes(x = flux_value, fill = landscape_position)) +
...
ggplot(df_BGC, aes(x = flux_value, fill = inundated)) +
  geom_histogram(aes(y = ..density..), alpha = 0.5, position = "identity", bins = 30) +
  geom_density(alpha = 0.2) +
...
leveneTest(flux_value ~ landscape_position, data = df_BGC) #p=0.18, ok 
leveneTest(flux_value ~ inundated, data = df_BGC)#p <0.001, NOT* homog of var


#for interactions
leveneTest(flux_value~ landscape_position * inundated, data = df_BGC) #p<0.001, not homog*


#test distribution normality of each numeric predictor variable 
shapiro.test(df_BGC$soil_temp_10_cm) #p=0.004, NOT normal** 
shapiro.test(df_BGC$thawdepth) #p = <0.001, NOT normal **
shapiro.test(df_BGC$VWC) #p<0.001, NOT normal **
shapiro.test(df_BGC$air_temp) #p<0.001, NOT normal **
```
...
cor.test(df_BGC$soil_temp_10_cm, df_BGC$thawdepth, method="spearman")#p=0.443 -> not correlated 
#soil temp v VWC
cor.test(df_BGC$soil_temp_10_cm, df_BGC$VWC, method="spearman") #p<0.001, sig* correlated 
#soil temp v air temp 
cor.test(df_BGC$soil_temp_10_cm, df_BGC$air_temp, method="spearman") #p<0.001, correlated **
#thaw depth v VWC
cor.test(df_BGC$thawdepth, df_BGC$VWC, method="spearman") #p<0.001, *sig*, correlated **
#thaw depth v air temp 
cor.test(df_BGC$thawdepth, df_BGC$air_temp, method="spearman") #p=0.59, not corr 
#air temp v VWC
cor.test(df_BGC$air_temp, df_BGC$VWC, method="spearman") #p=0.15, not corr 
...
gls_VWCinun <- gls(flux_value ~ landscape_position + VWC + inundated, 
                 ...
                 data = df_FCH4, method = "REML")
summary(gls_VWCinun) 
anova(gls_VWCinun) #landpos p<0.001, SIG*; VWC p = 0.038, inundated p<0.001 SIG*
