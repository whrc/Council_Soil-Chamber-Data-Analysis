...
anova(MW.NEE.final) #shows soil temp and thaw depth are sig p<0.05
summary(MW.NEE.final) #shows only soil temp is sig 
#sig diff in soil temp & thaw depth (p<0.001); land pos p=0.12; VWC p = 0.28)

```
####Multicollinearity in final model- EC.NEE
```{r}
#variance inflation factor (vif) - vif = 1 = variables are not correlated; between 1 & 5 = moderately correlated,  5 or higher indicates highly correlated & multicollinearity among variables. GVIF is a generalized VIF for categorical variables with more than two levels; the adj VIF gives values for comparing across variables with diff degrees of freedom* --> look at both when assessing collinearity in a model 
library(car)
vif(MW.NEE.final) #thawdepth is 21.09, and VWC is 26, so definitely multicollinearity there! 
```
#***TO DO: rework final model to remove multicollinearity! 
...
acf(residuals_gls, main = "Autocorrelation of Residuals") #want bars of Kag1 and higher to be below the blue-dotted lines of confidence -- ignore Lag0 ** 
#Result: no apparent autocorrelation -- Lag0 represents correlation of residuals with themselves and is always ~1, so this isn't useful and isn't a problem. Lag 1 shows corr between each residual and the prev residual, lag 2 = correlation between resisuals 2 steps apart, etc. **Focus on Lag1 and higher*
...
#Result: 0.102, suggests no autocorrelation
...
```
#non-parametric tests - GEE (generalized estimating equations) - can include continuous and categorical predictors, can incorporate var structure 
```{r}
library(geepack)
...
MW.NEE.gam <- gam(flux_value ~ landscape_position + s(soil_temp_10_cm) + s(thawdepth) + s(VWC), 
                  data = df_MWlongNEE, 
                  method = "REML", 
                  na.action = na.omit)
...
MW.NEE.rlm <- rlm(flux_value ~ landscape_position + soil_temp_10_cm + thawdepth + VWC, 
                  data = df_MWlongNEE, 
                  na.action = na.omit)

summary(MW.NEE.rlm) 
#Results: all coeffs = 0......wtf

#expand the view so you can see the full value of the coeffs
summary(MW.NEE.rlm)$coefficients
# or
options(digits=10)
summary(MW.NEE.rlm)

#Extract p-values based on t-stat and df
# For your specific t-values with 17 degrees of freedom
t_values <- c(-2.9119, 1.5749, 2.7041, 0.2993, 1.4269)
variable_names <- c("Intercept", "landscape_positionupland", "soil_temp_10_cm", "thawdepth", "VWC")

# Calculate two-sided p-values
p_values <- 2 * pt(abs(t_values), df = 17, lower.tail = FALSE)
...
# Print the result
result_table
#Result: soil temp is sig, p = -.015; others not sig * --> this matches the results of the final gls model EC.NEE.final 
```
