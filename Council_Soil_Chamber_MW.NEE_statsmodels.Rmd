...
```{r}
# Final MW NEE model: remove thawdepth and VWC - refit with REML 
MW.NEE.final <- gls(flux_value ~ landscape_position + soil_temp_10_cm, 
                           weights = varIdent(form = ~ 1| landscape_position),
                          data = df_MWlongNEE, method = "REML", na.action = na.omit)
anova(MW.NEE.final)
...
library(emmeans)
emmeans(MW.NEE.final, adjust = "Tukey", pairwise ~ landscape_position) #p=0.027 - only upland and lowland  


gls.MW.NEE.soiltemp = gls(flux_value ~ soil_temp_10_cm, data = df_MWlongNEE, method = 'ML', na.action=na.exclude)
anova(gls.MW.NEE.soiltemp) #no sig diff, p = 0.13
```
