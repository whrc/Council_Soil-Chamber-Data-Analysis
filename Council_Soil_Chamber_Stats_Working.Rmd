...
car::qqPlot(x = residuals(NEE.final))
```
...
df_GPP$soil_temp_10_cm = as.numeric(df_GPP$soil_temp_10_cm)
...
anova(gls.GPP.soiltemp) #p=0.93, not sig 
...
```
###**Variance Structure - RECO *** inundation and soil temp not working here 
```{r}
...
anova(var.RECO.landpos) #now showing sig diffs for soil temp (p=0.0007)
anova(gls.RECO, var.RECO.landpos) #p = 0.59 - var does not improve model 

#based on plot type 
var.RECO.plottype = update(gls.RECO, weights = varIdent(form = ~1|plot_type))
anova(var.RECO.plottype) #now showing sig diffs for soil temp, landscape pos, and inundated (p<0.01)
anova(gls.RECO, var.RECO.plottype) #p = <0.0001, adding in the variance by iplottype significantly improves the model fit (lower AIC, lower BIC, and higher LogLik)
...
#Multiple Var Structure - RECO  *inundation and soil temp won't work 
```{r}
#looking at models with multiple variance structures 
#inundation and land pos 
library(nlme)
